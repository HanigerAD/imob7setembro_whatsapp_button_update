...
export class ImageService {
    ...
    public async saveImages(images: ImageRequest[], res: Response): Promise<void> {
        if (images?.length) {
            const validImages = images.filter(image => this.acceptedFormats.includes(image.file.mimetype));
            const cdnDtos = await Promise.all(validImages.map(image => this.prepareImageForCDN(image)));
