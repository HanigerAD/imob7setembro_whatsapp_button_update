...
export class PropertyController {
    ...
    public insertImages(@Param('code') code: number, @UploadedFiles() files: Express.Multer.File[], @Res() res: Response): void {
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(HttpStatus.OK)
    public updateImagesSort(
        @Body() imagesSort: ImageSortRequest[]
    ): void {
    ...
    @Get('properties')
    public async getAllProperty(@Query() filter: PropertyFilterRequest, @Res() res: Response): Promise<any> {
        res.send(await this.service.getAllProperties(filter, res));
    }

    @Get('properties/:code')
    public getSingleProperty(@Param('code') code: number): Promise<PropertyDetailResponse> {
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(HttpStatus.OK)
    public updateProperty(@Param('code') code: number, @Body() request: PropertyRequest): Promise<number> {
    ...
    @HttpCode(HttpStatus.OK)
    @UseInterceptors(FilesInterceptor('files'))
    public insertDocuments(@Param('code') code: number, @UploadedFiles() files: Express.Multer.File[], @Res() response: Response) {
    ...
    public async getPropertyImageUrl(@Param('code') code: number, @Res() res): Promise<string[]> {
        const images = await this.service.getPropertyImagesUrls(code);
        return res.send(images);
    }

    @Get('properties/:code/documents')
    public async getPropertyDocuments(@Param('code') code: number, @Res() res): Promise<PropertyDocumentResponse[]> {
    ...
    public getValueLog(@Param('field') field: string, @Param('value') value: number): Promise<any> {
